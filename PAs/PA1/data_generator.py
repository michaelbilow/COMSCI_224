...
def make_individuals():
    n_cases = N_INDIVIDUALS/2
    n_controls = N_INDIVIDUALS/2

    cases = []
    controls = []

    indivs_per_round = n_cases
    while len(cases) < n_cases and len(controls) < n_controls:
        raw_indivs = norm.rvs(size=(indivs_per_round, N_SNPS))
        if CORR_MAT_SQRT:
        ...
        # print liabilities
        case_statuses = liabilities > CASE_THRESHOLD
        # print case_statuses.all()
        # print case_statuses.astype(int).sum()
        new_cases = genetic_state[case_statuses]
        # print new_cases
        new_controls = genetic_state[~case_statuses]
        cases.append(new_cases)
        controls.append(new_controls)
        cases_ascertained = sum((len(_) for _ in cases))
        print cases_ascertained, 'ascertained so far'
        if cases_ascertained > n_cases:
            break
