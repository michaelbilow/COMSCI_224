...
def write_geno_data(fn, geno_dict):
...
def write_pheno_data(fn, pheno_dict):
    # print pheno_dict.items()[:10]
    df = pd.Series(pheno_dict)
    df.to_csv(fn, sep=' ')
    # print df.mean()
    return


def write_association_data(fn, association_dict, zfill_amt):
    print association_dict.items()[:10]
    cleaning_lambda = lambda _: 'SNP_{{:0{}}}'.format(zfill_amt).format(_)
    cleaning_f = lambda _: cleaning_lambda(_) if isinstance(_, int) else '{}-{}'.format(cleaning_lambda(_[0]),
                                                                                        cleaning_lambda(_[1]))
    clean_dict = {cleaning_f(k): v for k, v in association_dict.items()}
    with open(fn, 'w') as f:
        for k, v in sorted(clean_dict.items()):
            f.write('{}:{}\n'.format(k, v))
    return


def make_data(folder_name, n_snps, generation_sizes, additional_unrelated_indivs, N_snp_effects, N_snp_snp_effects,
              n_populations=1):
    ...
    for dataset in ('example', 'test'):
        input_dicts = []
        for pop in range(n_populations):  ## Right now, only works with one population
        ...
        phenos_dict, association_dict = make_phenos(genotypes_dict=complete_input_dict,
                                                    N_snp_effects=N_snp_effects,
                                                    N_snp_snp_effects=N_snp_snp_effects)

        geno_input_fn = join(folder_name, dataset, 'input', 'genotypes.txt')
        pheno_input_fn = join(folder_name, dataset, 'input', 'phenotypes.txt')
        output_fn = join(folder_name, dataset, 'output', 'associations.txt')

        write_geno_data(fn=geno_input_fn, geno_dict=complete_input_dict)
        write_pheno_data(fn=pheno_input_fn, pheno_dict=phenos_dict)
        write_association_data(fn=output_fn, association_dict=association_dict,
                               zfill_amt=int(np.log10(len(phenos_dict) - .9)) + 1)


if __name__ == "__main__":
    make_data(folder_name='easy', n_snps=1000, generation_sizes=(300,),
              additional_unrelated_indivs=0, N_snp_effects=5, N_snp_snp_effects=15)
